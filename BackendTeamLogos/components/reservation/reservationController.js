...
const reservationController ={
    ...
    lookup_doktor: async(req, res) => {
        try {
            ...
            (err,result)=>{
                ...
                if (result.length === 0) {
                    return res.json({ status: false, message: 'No matching data found.' });
                }
    ...
    findSpecific: async(req, res) => {
        try {
            const { day, psyc_id, client_id, limit, skip, sort_by, sort_order, select_fields } = req.query;
            const sort = {};
    ...
    delete: async(req, res) => {
        try {
            const reservation = await reservationModule.findByIdAndDelete(req.params.id);
            res.json({
                status: true,
    ...
    lastDay: async(req, res) => {
      try {
        const timeChack = (itemTime) => {
          ...
          const itemDateTime = new Date(currentTime.getFullYear(), currentTime.getMonth(), currentTime.getDate(), (itemHour-3), itemMinute);
        ...
        const { day,client_id, psyc_id, limit, skip, sort_by, sort_order, select_fields } = req.query;
