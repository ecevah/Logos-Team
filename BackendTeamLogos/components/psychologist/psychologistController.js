...
const psychologistController = {
    login: async(req, res) => {
        try {
            const psychologist = await psychologistModel.findOne({ eMail: req.body.email });
    ...
    find: async(req, res) => {
        try {
            ...
            const psychologist = await psychologistModel.findById(req.params.id).limit(limit).skip(skip);
            res.json({
                status: true,
    ...
    findSpecific: async(req, res) => {
        try {
            let { name, surname, email , identity, psikolog,klinik_psikolog , aile_danismani,
            cift_terapisti, cinsel_terapist,psikolojik_danisman, çoçuk_gelişimci, uzman_psikolog,
            uzman_psikolojik_danisman,dr_ozel_egitim_uzmani,uzman_noropsikolog, limit, skip} = req.query;
            const psyc = await psychologistModel.find({
                $or: [
                    {name:name},
                    {surName: surname},
                    {eMail: email},
    ...
    /*create: async(req, res) => {
        try {
            const { name, surname, pass, email,tecrube,unvan,tag } = req.body;
    ...
    delete: async(req, res) => {
        try {
            const psychologist = await psychologistModel.findByIdAndDelete(req.params.id);
            res.json({
                status: true,
    ...
    update: async(req, res) => {
        try {
            const psychologist = await psychologistModel.findByIdAndUpdate(req.body.id, req.body, { new: true });
            res.json({
                status: true,
   ...
   /*passUpdate: async(req,res) => {
    ...
    createStar: async(req, res) => {
        try {
            ...
            const starAvg = (psyc.star.reduce((acc, curr) => acc + curr, 0) / psyc.star.length).toFixed(1).toString();
            if (!psyc.starAvg ) {
              psyc.starAvg = [];
